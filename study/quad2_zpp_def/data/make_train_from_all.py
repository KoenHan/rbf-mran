...
class QuadWxp :
    ...
    def calc_f(self, qq0qq3uuuu) :
        ...
        zpp = self.MAX_THRUST*(qq0qq3uuuu)/(self.MASS_OF_MACHINE*self.MAX_RPS_POW) - self.A_OF_GRAVITY
        return zpp
...
if __name__ == "__main__" :
    ...
    with open("train.txt", "w") as f:
        ...
        for idx, data in enumerate(datas) :
            ...
            q3 = float(data[3])
            if idx == 0:
                ppre_qq0qq3 = -1.0+2.0*q0*q0+2.0*q3*q3
                continue
            ...
            qq0qq3uuuu = pre_qq0qq3*uuuu
            zpp = quadwxp.calc_f(qq0qq3uuuu)
            tmp = "\t".join([str(zpp), str(qq0qq3uuuu)])
            f.write(tmp+"\n")
            pre_qq0qq3 = -1.0+2.0*q0*q0+2.0*q3*q3
